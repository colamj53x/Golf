...
function ShotCueSettingsCard() {
  ...
  const courseFields: Array<[keyof ShotCueCard, string]> = [['goal', 'Goal'], ['preShot', 'Pre-shot'], ['setup', 'Set-up'], ['look', 'Where to look'], ['swing', 'Swing'], ['fullClock', 'Full-shot swing size'], ['halfClock', 'Half-shot swing size'], ['courseCue', 'Simplified on-course cue']];

  return <Card><CardHeader><div className="flex flex-wrap items-start justify-between gap-3"><div><CardTitle>Practice Technique & On-Course Cues</CardTitle><CardDescription>Edit the complete Practice card and the separate condensed Course card. Full and Half technique is kept separate.</CardDescription></div><div className="flex gap-2">{editing && <Button variant="outline" onClick={() => { setDraft(cards); setEditing(false); }}>Cancel</Button>}<Button onClick={() => { if (editing) { saveShotCues(draft); toast.success('Shot cues saved'); } setEditing(!editing); }}>{editing ? <><Save className="mr-2 h-4 w-4" />Save</> : <><Pencil className="mr-2 h-4 w-4" />Edit</>}</Button></div></div></CardHeader><CardContent><Accordion type="single" collapsible className="w-full">{draft.map(card => <AccordionItem key={card.id} value={card.id}><AccordionTrigger><span><span className="font-semibold">{card.title}</span><span className="ml-2 text-xs font-normal text-muted-foreground">{card.appliesTo}</span></span></AccordionTrigger><AccordionContent className="space-y-6 pt-2">
    <div className="space-y-4"><h4 className="font-semibold">Full Practice technique</h4>{card.technique.map((note, index) => <div key={`${note.label}-${index}`} className="rounded-md border p-3"><Label>{note.label}</Label>{note.fullText !== undefined || note.halfText !== undefined ? <div className="mt-2 grid gap-3 sm:grid-cols-2"><div><div className="mb-1 text-xs text-muted-foreground">Full</div><Textarea value={note.fullText || ''} disabled={!editing} onChange={event => updateTechnique(card.id, index, 'fullText', event.target.value)} rows={3} /></div><div><div className="mb-1 text-xs text-muted-foreground">Half</div><Textarea value={note.halfText || ''} disabled={!editing} onChange={event => updateTechnique(card.id, index, 'halfText', event.target.value)} rows={3} /></div></div> : <Textarea className="mt-2" value={note.text} disabled={!editing} onChange={event => updateTechnique(card.id, index, 'text', event.target.value)} rows={3} />}</div>)}</div>
    <div className="space-y-4 border-t pt-5"><h4 className="font-semibold">Condensed Course card</h4>{courseFields.filter(([field]) => (field !== 'fullClock' && field !== 'halfClock') || card[field] !== undefined).map(([field, label]) => <div key={field} className="space-y-1.5"><Label>{label}</Label><Textarea value={(card[field] as string) || ''} disabled={!editing} onChange={event => update(card.id, field, event.target.value)} rows={field === 'courseCue' ? 2 : 3} /></div>)}</div>
