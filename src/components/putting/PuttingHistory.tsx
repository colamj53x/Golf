...
import { BlastMotionSetData, DrillResult, PuttingSessionRecord } from '@/types/putting';
...
export function PuttingHistory({ sessions, onChanged }: Props) {
  ...
  const handleSaveBlastMetrics = async (session: PuttingSessionRecord, result: DrillResult, blast: BlastMotionSetData) => {
    ...
    try {
      ...
      toast.success('Blast metrics saved');
      onChanged();
    } catch {
              ...
              <div key={s.id} className="rounded-md border">
                ...
                {expanded && (
                  <div className="space-y-2 border-t bg-muted/10 p-3">
                    <p className="text-xs text-muted-foreground">Open a drill to add or update its Blast Motion metrics. Existing drill scores are not changed.</p>
                        ...
                        <div key={result.drill_id} className="space-y-3 rounded-md border bg-background p-3">
                          <div className="flex flex-wrap items-center justify-between gap-3">
                            <div>
                              ...
                              <div className="text-xs text-muted-foreground">{result.final_score} / {result.max_score}</div>
                            </div>
                          </div>
                          <BlastMetricsEditor
                            value={result.blast}
                            disabled={savingDrillId === uploadKey}
